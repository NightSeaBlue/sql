...
-- Zlotkey는 제외하십시오.
SELECT FIRST_NAME||' '||LAST_NAME AS 이름 , HIRE_DATE
FROM EMPLOYEES
WHERE LAST_NAME NOT LIKE 'Zlotkey' AND DEPARTMENT_ID LIKE (SELECT DEPARTMENT_ID FROM EMPLOYEES WHERE LAST_NAME LIKE 'Zlotkey');
...
SELECT EMPLOYEE_ID,FIRST_NAME||' '||LAST_NAME AS 이름
FROM EMPLOYEES
WHERE DEPARTMENT_ID in (SELECT DEPARTMENT_ID FROM EMPLOYEES WHERE LOWER(FIRST_NAME) LIKE LOWER('%u%') or LOWER(LAST_NAME) LIKE LOWER('%u%'));
...
-- 4. 부서 위치 ID 가 1700 인 모든 사원의 이름, 부서 번호 및 업무 ID 를 표시하십시오.
SELECT E.FIRST_NAME||' '||E.LAST_NAME AS 이름, E.DEPARTMENT_ID , E.EMPLOYEE_ID
FROM EMPLOYEES E LEFT OUTER JOIN DEPARTMENTS D
                    ON E.DEPARTMENT_ID = D.DEPARTMENT_ID
WHERE E.DEPARTMENT_ID IN (SELECT D.DEPARTMENT_ID FROM DEPARTMENTS D WHERE LOCATION_ID = 1700);
...
FROM EMPLOYEES 
WHERE SALARY>(SELECT AVG(SALARY) FROM EMPLOYEES) AND DEPARTMENT_ID in (SELECT DEPARTMENT_ID FROM EMPLOYEES WHERE FIRST_NAME||' '||LAST_NAME LIKE '%u%')
ORDER BY SALARY;
